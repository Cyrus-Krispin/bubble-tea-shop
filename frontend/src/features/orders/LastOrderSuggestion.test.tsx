...
function renderSuggestion(
  session: { accessToken: string; expiresAt: number; email: string } | null,
...
describe("LastOrderSuggestion", () => {
  ...
  it("disables quick add until at least one saved drink is selected", async () => {
    renderSuggestion({ accessToken: "customer-token", expiresAt: 4102444800, email: "customer@example.test" });
  ...
  it("keeps a conflicting cart unchanged when selected drinks cannot be added", async () => {
    renderSuggestion(
      { accessToken: "customer-token", expiresAt: 4102444800, email: "customer@example.test" },
      { seedConflict: true },
    );
  ...
  it("is hidden for guests and customers without an eligible latest order", async () => {
    ...
    rerender(
      <AuthContext.Provider value={{
        isLoading: false,
        session: { accessToken: "customer-token", expiresAt: 4102444800, email: "customer@example.test" },
      }}>
        <MemoryRouter>
          <CartProvider>
            <LastOrderSuggestion locationSlug="orchard-central" products={catalogProducts} />
