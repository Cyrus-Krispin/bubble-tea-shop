...
inventory movements, order status transitions, and owner-managed staff access changes. It helps owners and managers answer who changed
...
- Owners see organization-wide catalog events and operational events from every organization
  location, plus staff-access lifecycle events.
...
(`CATALOG`, `INVENTORY`, `ORDER`, or owner-only `STAFF`) and bounded `page`/`size` parameters. Events are ordered by
